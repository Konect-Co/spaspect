...
def draw_bounding_box_on_image(image,
  ...
  for display_str in display_str_list[::-1]:
    ...
    draw.rectangle(
        [(xmin, text_bottom - text_height - 2 * margin), (xmin + text_width,
                                                          text_bottom)],
        fill=color)
    draw.text(
...
def visualize_boxes_and_labels_on_image_array(
  ...
  for box, color in box_to_color_map.items():
    ...
    draw_bounding_box_on_image_array(
        ...
        use_normalized_coordinates=use_normalized_coordinates)
    print(xmax*image.shape[1],ymin*image.shape[0])
    print(xmax*image.shape[1],ymax*image.shape[0])
    print(xmin*image.shape[1],ymin*image.shape[0])
    print(xmin*image.shape[1],ymax*image.shape[0])
    print('')
    midpoint = ((xmax*image.shape[1]+xmin*image.shape[1])*0.5,ymax*image.shape[0])
    print("Midpoint of bottom edge is: ",midpoint)
    print('')
    if keypoints is not None:
